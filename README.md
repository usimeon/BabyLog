...
EXPO_PUBLIC_GOOGLE_DRIVE_IOS_CLIENT_ID=YOUR_GOOGLE_DRIVE_IOS_OAUTH_CLIENT_ID
EXPO_PUBLIC_GOOGLE_DRIVE_ANDROID_CLIENT_ID=YOUR_GOOGLE_DRIVE_ANDROID_OAUTH_CLIENT_ID
...
- Configure OAuth apps in provider consoles:
  - Google: create native OAuth clients (not Web):
    - iOS client with bundle id `com.example.babylog` -> `EXPO_PUBLIC_GOOGLE_DRIVE_IOS_CLIENT_ID`
    - Android client -> `EXPO_PUBLIC_GOOGLE_DRIVE_ANDROID_CLIENT_ID`
...
- In app:
  ...
  - Choose destination
  - Tap `Drive` or `Dropbox` once to connect
  - Run backup
